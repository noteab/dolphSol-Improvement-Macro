...
def check_modules_and_tesseract():
    pip_check_output = subprocess.run([sys.executable, "-m", "pip", "check"], capture_output=True, text=True)
    installed_modules = pip_check_output.stdout.splitlines()
    
    missing_modules = []
    
    for line in installed_modules:
        for module in all_required_modules:
            if module in line:
